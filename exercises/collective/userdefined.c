...
void sumcompound(void *invec, void *inoutvec, int *len, MPI_Datatype *datatype)
{
  int i, rank;

  struct compound *compinoutvec, *compinvec;

  compinvec    = (struct compound *) invec;
  compinoutvec = (struct compound *) inoutvec;

  for (i=0; i < *len; i++)
    {
      compinoutvec[i].ival = compinoutvec[i].ival + compinvec[i].ival;
      compinoutvec[i].dval = compinoutvec[i].dval + compinvec[i].dval;
    }
...
int main(){
  ...
  MPI_Op_create(sumcompound, 1, &MPI_COMPOUND_SUM);
  ...
  MPI_Allreduce(&passon, &sum, 1, newtype, MPI_COMPOUND_SUM, comm);

  printf( "The new sums are: %d %f on processor %d \n",sum.ival,sum.dval, rank);
  MPI_Finalize();
